...
//add for tp gesture node start

int focaltech_gesture_enable = 0;
static struct kobject *FTS_tp_device;
...
{
     int count;
     count = sprintf(buf, "focaltech_gesture_enable =  %d\n", focaltech_gesture_enable);
     return count;
}
...
static int tp_gesture_creat_sys_entry(void)
{
	...
	FTS_tp_device = kobject_create_and_add("android_touch", NULL);
	if (FTS_tp_device == NULL) {
		printk(" %s: subsystem_register failed\n", __func__);
		rc = -ENOMEM;
		return rc ;
	}
	rc = sysfs_create_file(FTS_tp_device, &dev_attr_wakeup_gesture_enable.attr);
	if (rc) {
		printk(" %s: sysfs_create_file failed\n", __func__);
		kobject_del(FTS_tp_device);
	}
...
void fts_irq_disable(void)
{
    ...
    if (!fts_wq_data->irq_is_disable)
    {
        disable_irq(fts_wq_data->client->irq);
        fts_wq_data->irq_is_disable = 1;
    }
}
...
#if FTS_GESTURE_EN
    if (fts_gesture_resume(data->client) == 0 && focaltech_gesture_enable == 1)
    {
